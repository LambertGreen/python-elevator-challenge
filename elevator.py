...
class ElevatorLogic(object):
    ...
    def on_called(self, floor, direction):
        ...
        The elevator could be requested at any floor at any time, going in either
        direction.
        ...
        if (
            self._is_pick_up_requested_on_floor(floor, direction)
            or (floor == 1 and direction == DOWN)
            or (floor == FLOOR_COUNT and direction == UP)
        ):
            return
    ...
    def on_floor_selected(self, floor):
        ...
        if self.service_direction is None:
            ...
            # if a switch of direction is allowed and this request is in the new
            # direction thenthe switch can occur
            if (
                self.is_ready_to_switch_direction
                and direction_of_request == self._get_opposite_service_direction()
            ):
                self.service_direction = self._get_opposite_service_direction()
                self.drop_off_floors.append(floor)
                self.is_ready_to_switch_direction = False

            # only accept drop off requests to floors that are ahead in the current
            # service direction
            elif (
                not self.is_ready_to_switch_direction
                and direction_of_request == self.service_direction
            ):
                self.drop_off_floors.append(floor)
    ...
    def on_floor_changed(self):
        ...
        #       the elevator has no more service requests in the current service
        #       direction
        ...
        # check if the elevator's current location and direction matches a service
        # request
        if self._is_pick_up_requested_on_this_floor(self.service_direction):
            # stop here for pick up
            self.callbacks.motor_direction = None

        # check if the elevator has no more requests ahead in the current service
        # direction and can stop
        # here to pick up a request to go in the opposite direction
        elif not self._are_more_drop_off_or_pickup_requests_ahead(
            self.service_direction
        ):
            if self._is_pick_up_requested_on_this_floor(
                self._get_opposite_service_direction()
            ):
                # stop here for pick up
                self.callbacks.motor_direction = None
        ...
        assert not (
            self.callbacks.current_floor == 1 and self.callbacks.motor_direction == DOWN
        )
        assert not (
            self.callbacks.current_floor == FLOOR_COUNT
            and self.callbacks.motor_direction == UP
        )
    ...
    def on_ready(self):
        ...
        """
        # If there are no outstanding service requests the motor direction is set to
        # none and the elevator will entire idle mode.
        # If there are  more service requests in the current service direction then
        # the motor must continue in this direction.
        # If there are instead request in the other direction then the motor
        # should be set to this other direction
        ...
        # Check if the elevator only has a pick up request in the service direction
            ...
            elif self._are_more_drop_off_or_pickup_requests_ahead(
                self._get_opposite_service_direction()
            ):
                self.service_direction = self._get_opposite_service_direction()
                self.callbacks.motor_direction = self.service_direction
        ...
        elif self._is_pick_up_requested_on_this_floor(
            self._get_opposite_service_direction()
        ):
    ...
    def _is_idle(self):
        return (
            len(self.drop_off_floors) == 0
            and len(self.pick_up_floors_by_direction[UP]) == 0
            and len(self.pick_up_floors_by_direction[DOWN]) == 0
        )

    def _are_more_drop_off_or_pickup_requests_ahead(self, direction):
        return self._are_more_drop_off_requests_ahead(
            direction
        ) or self._are_more_pick_up_requests_ahead(direction)
    ...
    def _is_pick_up_requested_on_this_floor(self, direction):
        return self._is_pick_up_requested_on_floor(
            self.callbacks.current_floor, direction
        )
    ...
    def _are_more_drop_off_requests_ahead(self, direction):
        ...
        if direction == UP:
            return (
                len(self.drop_off_floors) > 0
                and max(self.drop_off_floors) > self.callbacks.current_floor
            )
        else:
            return (
                len(self.drop_off_floors) > 0
                and min(self.drop_off_floors) < self.callbacks.current_floor
            )
    ...
    def _are_more_pick_up_requests_ahead(self, direction):
        ...
        if direction == UP:
            return (
                ...
                len(self.pick_up_floors_by_direction[UP]) > 0
                and max(self.pick_up_floors_by_direction[UP])
                ...
                and max(self.pick_up_floors_by_direction[DOWN])
                > self.callbacks.current_floor
            )
        else:
            return (
                ...
                len(self.pick_up_floors_by_direction[UP]) > 0
                and min(self.pick_up_floors_by_direction[UP])
                < self.callbacks.current_floor
            ) or (
                # going down there is a floor ahead with a DOWN request
                len(self.pick_up_floors_by_direction[DOWN]) > 0
                and min(self.pick_up_floors_by_direction[DOWN])
                < self.callbacks.current_floor
            )
